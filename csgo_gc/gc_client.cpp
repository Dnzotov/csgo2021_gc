...
// (BuildReservationPayload, the client side builder of the "G<cookie>,<matchid>,1:[<account>]" payload, is gone with the
// client side reservation, RESEARCH_FINDINGS.md #61; the server side payload format lives in reservation_keepalive.h.)
...
void ClientGC::OnBackendSearchResult(const std::vector<uint8_t> &buffer)
{
    ...
    if (m_pendingAccept.active && result.requestId == BackendClient::ActiveRequestId())
    {
        ...
        {
            EndAccept(result.status);
        }
        else
        {
            WithdrawAccept("the search is " + result.status + " again");
        }
    ...
    {
        Platform::Print("[MM] backend: a match is gathering players (no server is reserved before it is full), %u/%u\n",
            result.matchPlayers, result.matchRequired);
    }
    else
    {
...
void ClientGC::StartServerFlow(const BackendClient::Assignment &assignment, bool alreadyAccepted)
{
    ...
    // Non-Accept modes (Casual, Deathmatch, ...): nothing left to do here. The server the backend assigned is a dedicated
    // srcds in ANOTHER process that reserves itself (ServerGC / ReservationKeepAlive, RESEARCH_FINDINGS.md #60), and the
    // client reaches it through QueueConnect. This process used to reserve its OWN engine server here as well
    // (IVEngineServer::ReserveServerForQueuedGame, prototype leftover of #31); it never reached the assigned server, but the
    // cookie stayed on this process' `sv` (nothing expires it there) and the next local Training/bots game was rejected by
    // CBaseServer::ConnectClient: "mismatching cookie from loopback, client 0, server 293a206f6c6c6548" -> "Invalid user
    // info" (RESEARCH_FINDINGS.md #61). offline_tests/reservation/client_flow_test.cpp guards this.
}

// MatchmakingGC2ClientUpdate (9104) `matchmaking` values, from the client's own handler (client_panorama.dll sub_103D9900,
// RESEARCH_FINDINGS.md #35/#37): 3 = reset the ready-up (RaiseReadyUp(false, 0, 0): the Match Found popup closes) and show
// "searching"; 0 = clear the matchmaking state (the queue marker goes away).
static constexpr int32_t MatchmakingUpdateSearching = 3;
static constexpr int32_t MatchmakingUpdateClear = 0;

void ClientGC::SendMatchmakingUpdate(int32_t matchmaking)
{
    CMsgGCCStrike15_v2_MatchmakingGC2ClientUpdate update;
    update.set_matchmaking(matchmaking);
    SendMessageToGame(false, k_EMsgGCCStrike15_v2_MatchmakingGC2ClientUpdate, update);
}

// The backend took the match back (the Accept deadline passed, a player left it): close the Match Found popup through the
// client's own ready-up reset and carry on searching - the backend keeps the same search and gathers a new match for it.
void ClientGC::WithdrawAccept(const std::string &reason)
{
    Platform::Print("[MM-ACCEPT] the backend withdrew match %s (%s): closing the Accept popup, the search goes on\n",
        m_pendingAccept.matchId.c_str(), reason.c_str());

    m_pendingAccept = {};
    AcceptTest::DisarmClient();
    m_activeSearch.active = true; // the same search is being served again (it is polled under the same request id)

    SendMatchmakingUpdate(MatchmakingUpdateSearching);
}

// The backend ended the search while its Accept was out (an admin removed it, ...): close the popup and clear the queue state.
void ClientGC::EndAccept(const std::string &reason)
{
    Platform::Print("[MM-ACCEPT] the backend ended the search of match %s (%s): closing the Accept popup\n",
        m_pendingAccept.matchId.c_str(), reason.c_str());
    ...
    SendMatchmakingUpdate(MatchmakingUpdateSearching); // the popup closes on this one ...
    SendMatchmakingUpdate(MatchmakingUpdateClear);     // ... and the queue marker goes away
}
